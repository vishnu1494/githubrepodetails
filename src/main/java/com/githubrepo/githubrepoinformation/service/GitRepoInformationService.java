...
public class GitRepoInformationService {
	...
	public GitHubResponseEntity getRepoDetailsForUser(String userId) {
		LOGGER.debug("GitRepoInformationService start");
		...
		try {
			...
			if (!CollectionUtils.isEmpty(repoList)) {
				...
				repoList.forEach(repo -> {
					...
					try {
						...
						branches = FindGitHubRepoUtil
								.getResponseForApiAsString(String.format(githubBranchUrl, userId, repo.getName()));
			...
			respObj.setStatus(Integer.toString(HttpStatus.NOT_FOUND.value()));
			return new GitHubResponseEntity(respObj, HttpStatus.NOT_FOUND);

		} catch (Exception ex) {
			LOGGER.error(ex.getMessage(), ex);
			respObj.setMessage(ex.getMessage());
			respObj.setStatus(Integer.toString(HttpStatus.INTERNAL_SERVER_ERROR.value()));
			return new GitHubResponseEntity(respObj, HttpStatus.INTERNAL_SERVER_ERROR);

		}
		LOGGER.debug("GitRepoInformationService end");
